...
async function run(label, path) {
  ...
  const results = []; for (const name of ['hotkey', 'distinct-miss-surge', 'scan-oversized-payload', 'shifting-hotset', 'redis-outage-no-l2', 'uneven-three-replica']) results.push(await workload(mod, name, rng(seed)));
