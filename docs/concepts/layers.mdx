...
Because L1 lives in the process, it is per-instance. Three servers means three L1 caches that know nothing about each other, which is the problem [invalidation](/concepts/invalidation) exists to solve.
...
L2 stores bytes, not objects, so values are encoded on the way in and decoded on the way out. That encoding is described in [serialization](/concepts/serialization).
