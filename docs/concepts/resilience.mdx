...
<Warning>
  **`enabled: true` is required, and it is not implied by anything else.** The check is literally `failSafe.enabled === true`. A `staleTtlMs` on its own does nothing at all: no value is ever remembered as stale, no fallback ever fires, and nothing warns you. Always write both.
</Warning>
