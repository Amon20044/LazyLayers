...
The built-in L1 and Redis L2 both retain tagged serializer values. Matching
`levels.L1.codec` and `levels.L2.codec` policies reuse one encoded buffer.
Different policies encode independently, while every reader still dispatches by
the tag and decodes each supported format. Invalidation events use the
serializer too, but wrap their own event envelope.
