...
The built-in L1 and Redis L2 both retain the same tagged serializer format. A value is encoded once on the matching-format path, copied into owned storage, and decoded on each read. Invalidation events use the serializer too, but wrap their own event envelope.
