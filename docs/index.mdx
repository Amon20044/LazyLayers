...
<Note>
  The name is the design principle. LazyLayers does the least work that still returns the right answer: **load late** so nothing is cached until somebody asks, **load once** so ten thousand concurrent callers produce one loader call, and **load small** so values travel as packed binary. New to this? Start with <a href="/docs/learn">the reading path</a>.
</Note>
