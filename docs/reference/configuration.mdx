...
<ParamField path="failSafe.staleTtlMs" type="number">
  No default. How long a value stays usable as a stale fallback after it is remembered. **Fail-safe does nothing without a positive value here**, even with `enabled: true`, because no stale copy is ever kept.
