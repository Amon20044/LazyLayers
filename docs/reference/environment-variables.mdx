...
The library reads eighteen environment variables. Three affect the process globally, fifteen configure the observability dashboard.
...
Reading is unaffected. Every encoding carries a four byte prefix, so values written as msgpack before you set this keep decoding normally, and values written as JSON keep decoding after you unset it. What you pay is size and speed on everything written while it is on, which is why neither variable belongs in production.

There is no constructor option for these two. The encoding is not configurable in code.
