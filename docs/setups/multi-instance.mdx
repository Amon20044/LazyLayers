...
If you cannot tolerate a missed event, the durable transports are laid out tab by tab on the [production checklist](/docs/guides/event-buses).
...
Running under Node.js clustering makes this sharper, because a single machine now hosts several caches that must not be confused with each other. The [production checklist](/docs/setups/serverful) works through it.
