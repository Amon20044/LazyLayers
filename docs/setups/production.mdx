...
Grant the cache user `+info` for this check. Grant the commands used by your configured store and event bus: `+ping`, `+get`, `+set`, `+pttl`, `+exists`, `+scan`, `+zadd`, `+zrem`, `+zcard`, `+zrange`, `+zscan`, `+unlink` or `+del`, `+eval`, `+pexpire`, and `+publish`, `+subscribe`, and `+unsubscribe` when Redis Pub/Sub is enabled. Restrict key and channel patterns to the cache namespace, for example `~billing-api:cache:*` and `&billing-api:cache:events`.
...
Lazy Layers owns its local invalidation trust lifecycle, so v0.5.2 does not enable Redis `CLIENT TRACKING`. Redis memory limits and eviction remain deployment settings; L1 pressure never rewrites Redis server policy.

Redis Search, vector indexes, embeddings, semantic caching, and AI features are not probed or initialized by the v0.5.2 cache core.
