...
A read now checks L1, then L2. A hit in L2 is written into L1 on the way back, so the next read on that instance stays in memory. A restart no longer starts cold, because L2 survives it.

Values are encoded with MessagePack on the way into Redis. You do not configure that; it is the default wire format and it is covered in [serialization](/concepts/serialization).
...
JetStream keeps a durable consumer per instance, so events that arrived while an instance was down are delivered when it reconnects. [Choosing an event bus](/guides/event-buses) compares all four transports.
...
<Columns cols={2}>
  <Card title="Failure handling" icon="shield" href="/guides/failure-handling">
    Stale fallback, negative caching and loader timeouts.
  </Card>
  <Card title="Event ordering" icon="list-ordered" href="/architecture/event-ordering">
