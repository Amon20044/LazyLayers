...
On a healthy hit the encoded value is written into L1 **before** being returned, then remembered as stale, then `hit` with level `L2` is emitted. Under memory pressure or without enough room in the local byte target, `promotion:bypassed` is emitted and the L2 value is returned without growing L1.
</Step>
...
The whole map is swept and entries past their `expiresAt` are removed. Unless explicitly set, `inflight.ttlMs` covers the loader hard timeout, the distributed wait budget when locking is active, and a 50 ms margin, with a 5,000 ms minimum. Default lifetimes are 10,050 ms without locking and 20,100 ms with Redis locking.
</Step>
...
```mermaid title="Write and Broadcast Pipeline"
sequenceDiagram
...
This runs between in-flight registration and the loader whenever the L2 supports locking, unless locking is explicitly disabled. Cold starts and cache expiry use this same path.

<Steps>

<Step title="Try to acquire">
A random token identifies the owner. An initial Redis failure falls through to the existing fail-open loader path. A lock held by another instance starts a bounded wait instead.
</Step>

<Step title="Recheck and load while renewing">
After acquisition, the cache is read again so a value published since the original miss avoids another load. RedisStore renews the token's lease at roughly one third of its lifetime. A separate local expiry deadline bounds a hung renewal. Detected ownership loss aborts the loader signal and discards a late loader result. Cleanup stops renewal and releases only this token's lock.
</Step>

<Step title="Wait for a result or retry acquisition">
Waiters sleep up to `pollMs`, then reread the layers and retry the lock if the value is still missing. A failed or crashed owner can be replaced by a waiter. Once contention has been observed, later Redis errors do not trigger an automatic unlocked load.
</Step>

<Step title="Bound the wait without starting another load">
The default budget is `max(lock ttlMs, loader hardMs) + pollMs`, initially 10,050 ms. At the deadline the cache emits `lock:timeout`, returns eligible stale data or throws `DistributedLockTimeoutError`. An explicit `onTimeout: 'load'` restores the unlocked fallback. Redis command latency is governed by your Redis client timeouts.
</Step>
...
<Step title="Encode and write L1">Awaited. The built-in store owns the encoded buffer and enforces byte admission before retaining it.</Step>
<Step title="Write L2 through the circuit breaker">Awaited. A matching-format Redis store reuses the encoded value. A failure emits `l2:error` and is swallowed, so the L1 write stands alone.</Step>
<Step title="Remember it as stale">Only when fail-safe is enabled, `staleTtlMs` is positive, and the shared stale-byte budget permits it.</Step>
