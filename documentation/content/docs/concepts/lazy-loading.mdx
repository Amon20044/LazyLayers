...
A cold L1 after a deploy, an L1 eviction, and an expired value all create the same first-reader problem. L1 expiry does not necessarily run the loader because L2 can refill it. A cold or expired L2 key does. [Stampede protection](/docs/guides/stampede-protection#what-happens-on-a-cold-or-expired-key) explains how the cache coordinates that case across servers.
...
```mermaid title="In-flight Deduplication"
sequenceDiagram
...
```mermaid title="Lazy Invalidation Fan-Out"
sequenceDiagram
    participant DB as Database
