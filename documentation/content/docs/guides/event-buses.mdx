...
For Cloudflare KV, the [Cloudflare setup](/docs/setups/cloudflare-kv) uses an application Queue to retry deletion in the shared KV namespace. A Queue delivers each message to one consumer; it does not broadcast to every Worker isolate's L1. Cloudflare Event Subscriptions only report supported platform events such as namespace lifecycle and build status. They do not emit cache key invalidations.
