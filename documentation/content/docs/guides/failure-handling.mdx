...
Fail-open does not guarantee a successful request. Loader failures, origin overload, contention deadlines, and detected lease loss can still reach the caller when no eligible stale value exists. [Stampede protection](/docs/guides/stampede-protection) explains the distinction between a Redis transport failure and a contended or lost lease.
