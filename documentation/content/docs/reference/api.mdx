---
title: "API"
description: "Managed production setup, every LazyLayersCache method, and the complete export surface."
---
...
| Task | Method |
| --- | --- |
| Create and connect a managed cache | [`setupCache`](#production-setup) |
| Read through the cache | [`getOrSet`](#getorset) |
| Warm a key | [`prewarm`](#prewarm) |
| Invalidate after a write | [`invalidate`](#invalidate) |
| Invalidate a key family | [`invalidateByPattern`](#invalidatebypattern) |
| Release resources | [`close`](#lifecycle-methods) |

See [configuration](/docs/reference/configuration) for option defaults and the [quickstart](/docs/quickstart) for an application example.
...
[Transaction coordination](/docs/reference/transactions) documents the exact import, client requirements, result unions, recovery flow, and the database, provider, and outbox responsibilities that remain outside the library.
...
For Cloudflare KV on Node.js, pass `kv: { namespace }` with a KV binding-compatible client such as `CloudflareKVRestNamespace`. An explicit KV selection suppresses implicit Redis discovery. Workers use the separate `lazy-layers-cache/cloudflare` entrypoint. See [Cloudflare KV setup](/docs/setups/cloudflare-kv).
...
<ParamField path="redis" type="SetupRedisOptions | false">
...
<ParamField path="eventBus" type="EventBus | false">
...
<ParamField path="invalidationPublisher" type="CloudflareInvalidationPublisher">
  Publishes `invalidate` and `invalidateByPattern` operations to a dedicated Cloudflare Queue after local deletion. The separate consumer retries KV deletion. Queue delivery does not fan out to every L1. See [Cloudflare KV setup](/docs/setups/cloudflare-kv#application-invalidation-queue).
</ParamField>

<ParamField path="startup.requireHealthy" type="boolean" default="true">
  Requires configured shared infrastructure and the inbound subscription to be ready before setup returns.
</ParamField>

<ParamField path="startup.timeoutMs" type="number" default="10000">
  Bounds each startup health, connection, and subscription readiness check.
</ParamField>

All `LazyLayersCacheOptions` are accepted alongside these setup fields. See [production setup](/docs/setups/production) for the resolved defaults.
...
<Steps>
  ...
  <Step title="Coordinate across instances">
...
<AccordionGroup>
  <Accordion title="Cache and stores">
    `ManagedLazyLayersCache`, `LazyLayersCache`, `HybridCache`, `MemoryStore`, `RedisStore`, `CloudflareKVStore`, `CloudflareKVRestNamespace`, `CircuitBreaker`, `setupCache`, `createCache`, and `CacheSetupError`.
  ...
  <Accordion title="Serializer">
    ...
    Shared cache records, including Cloudflare KV, use `serializeCacheValue` and `deserializeCacheValue`. Import them from `lazy-layers-cache` on Node or `lazy-layers-cache/cloudflare` in a Worker. That facade writes `HC1M`, `HC1G`, or `HC1J` with the shared 1 KiB gzip floor and 15% savings rule. The Node `serialize` function remains the full codec set for Redis and L1, including LZ4, Zstd, and Snappy.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="Cache and options">
    `SetupCacheOptions<K, V>`, `SetupRedisOptions`, `SetupCloudflareKVOptions`, `CacheStartupOptions`, `LazyLayersCacheOptions<K, V>`, `HybridCacheOptions<K, V>`, `HybridCacheResilienceOptions`, `CacheOptions`, `CacheLevelOptions`, `CacheLevel`, `CacheKey`, `CacheLayer<K, V>`, `CacheEntry<V>`, `CacheStore<K, V>`, `CacheLoader<V>`, `InflightEntry<K, V>`, `InflightOptions`, `InflightStore<K, V>`, `DistributedLock`, `DistributedLockOptions`, `CircuitBreakerOptions`, `CircuitBreakerState`, `RedisStoreOptions`, `CloudflareKVStoreOptions`, `CloudflareKVRestOptions`, `CloudflareKVNamespace`, `CacheLoggerOptions`, `CacheRuntimeEnv`.
