...
`setupCache` is the production entry point for Node.js. It accepts the full `LazyLayersCache` option surface, resolves L1, a configured Redis or Cloudflare KV L2, and an optional event bus, then applies the bounded setup defaults called out below. The subset that also exists on `CacheOptions` can be overridden per call on `getOrSet` or `prewarm`.
...
<ParamField path="levels.L2" type="{ maxEntries?: number; ttlMs?: number; codec?: CacheCodecOptions }">
...
<ParamField path="l2" type="CacheStore<K, V> | false">
  The shared store. `setupCache` creates a `CloudflareKVStore` when `kv` is set, otherwise a `RedisStore` when a Redis client or URL is available. An explicit `l2` takes precedence. The low-level constructor has no L2 default.
</ParamField>

<ParamField path="kv" type="SetupCloudflareKVOptions | false">
  Pass `{ namespace: CloudflareKVNamespace, store?: CloudflareKVStoreOptions }` to select Cloudflare KV for L2. An explicit KV selection suppresses implicit `REDIS_URL` discovery. Setup checks the namespace with a bounded `list` request and defaults L2 serialization to Worker-compatible HC1 MessagePack with adaptive gzip. `store.compression` defaults to `auto`; set it to `none` to skip compression attempts. Set `redis` explicitly if you also need its Pub/Sub bus. [Cloudflare KV setup](/docs/setups/cloudflare-kv) covers Workers, Node.js, and cost trade-offs.
</ParamField>

<ParamField path="invalidationPublisher" type="CloudflareInvalidationPublisher">
  Optional managed-cache Queue publisher for application invalidations. It acts after local deletion. Node.js can use `CloudflareQueueRestSender`, while a Worker uses a Queue binding. [Application invalidation Queue](/docs/setups/cloudflare-kv#application-invalidation-queue) explains the consumer and delivery limits.
</ParamField>
...
<ParamField path="levels.L1.codec / levels.L2.codec" type="CacheCodecOptions">
...
| Mode | Packed-size tiers |
| --- | --- |
| `auto` | None below 256 B, LZ4 below 4 KiB, then Zstd when available or LZ4 otherwise. This is the shipped default |
| `none` | No compression |
...
<ParamField path="CompressionTier.maxBytes" type="number">
...
<ParamField path="CompressionTier.codec" type="'none' | 'gzip' | 'zstd' | 'lz4' | 'snappy'">
...
`serializeWithStats(value, policy)` reports encoding, original bytes, stored bytes, and compression savings. `deserialize` returns `null` for corrupt data or an unavailable decoder, so a built-in store treats that entry as a miss. See [serialization](/docs/concepts/serialization) for compatibility and measurement guidance.
...
<ParamField path="observability.route" type="string" default="/__lazylayers">
