...
`acquireLock`, `renewLock` and `releaseLock` back `distributedLock`. Acquire is `SET NX PX` on the lock key. Renewal uses Lua to compare the token before extending expiry with `PEXPIRE`. Release compares the token before deleting. An expired owner cannot renew or release another owner's lock. HybridCache manages renewal automatically.
