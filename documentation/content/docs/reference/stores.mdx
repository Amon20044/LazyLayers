...
The two runtimes share an `LLK1` envelope with a logical expiry and the existing HC1 MessagePack format. The Node store and `setupCache({ kv })` select Worker-compatible HC1 serialization by default. Both KV stores try gzip for packed values of at least 1 KiB and keep it only when gzip saves at least 15% of packed bytes; `compression: 'none'` disables new compressed writes. Raw and compressed KV records use `HC1M` and `HC1G` and are readable from either runtime. KV operation charges do not shrink with the payload. `CloudflareQueueInvalidationPublisher` can publish application invalidations from either runtime, and a dedicated Queue consumer repeats KV deletion. See [Cloudflare KV setup](/docs/setups/cloudflare-kv) for the system design and limits.
