...
Worker and Node KV stores share the existing HC1 MessagePack format. Values below 1 KiB stay as raw `HC1M` MessagePack. For packed values at least 1 KiB, `compression: 'auto'` (the default) tries gzip; Workers use the web `CompressionStream` API and Node uses the existing serializer. It stores `HC1G` only if gzip saves at least 15% of the packed bytes. Otherwise it keeps `HC1M`. Both readers decode `HC1M` and `HC1G`, and Worker decompression is capped at 25 MiB. They can also read earlier `HC1J` records. Set `compression: 'none'` for already compressed or incompressible values where the trial would waste CPU. This option changes new writes; readers continue to accept both formats.
