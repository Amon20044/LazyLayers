...
`invalidate` and `invalidateByPattern` delete KV and publish a durable application invalidation. The consumer retries the deletion. Queue delivery does not clear L1 in every isolate. See the [Cloudflare KV guide](../../documentation/content/docs/setups/cloudflare-kv.mdx) for limits and pattern deletion behavior.
