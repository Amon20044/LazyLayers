...
function parseLimit(value: MemoryLimit, capacity: number): number {
  ...
  if (percent) { const p = Number(percent[1]); if (p <= 0 || p > 100) throw new RangeError('Memory percentage must be greater than 0 and no greater than 100'); return capacity * p / 100; }
...
export class MemoryBudget {
  ...
  constructor(options: MemoryBudgetOptions) {
    ...
    if (options.sampleIntervalMs !== undefined && (options.sampleIntervalMs < 0 || !Number.isFinite(options.sampleIntervalMs))) throw new RangeError('sampleIntervalMs must be positive or zero');
    this.hardCap = Math.floor(parseLimit(options.maxMemory, capacity));
    if (!Number.isFinite(this.hardCap) || this.hardCap <= 0 || !Number.isInteger(this.hardCap)) throw new RangeError('maxMemory must resolve to a positive integer');
    this.minMemory = options.minMemory === undefined ? undefined : Math.floor(parseLimit(options.minMemory, capacity));
    if (this.minMemory !== undefined && (this.minMemory <= 0 || !Number.isInteger(this.minMemory) || this.minMemory > this.hardCap)) throw new RangeError('minMemory must be a positive integer no greater than maxMemory');
  ...
  tryReserve(bytes: number, category: MemoryCategory = 'fresh'): boolean {
    if (!Number.isFinite(bytes) || !Number.isInteger(bytes) || bytes < 0) return false;
  ...
  release(bytes: number, category: MemoryCategory = 'fresh'): void {
    if (!Number.isFinite(bytes) || !Number.isInteger(bytes) || bytes < 0) return;
  ...
  sample(signals?: MemorySignals | MemorySignalReader): MemoryBudgetSnapshot {
    this.signals = signals && 'available' in signals && ('effectiveBytes' in signals || 'availableBytes' in signals)
      ? signals as MemorySignals : readMemorySignals(signals as MemorySignalReader ?? this.opts.memory);
    const now = this.now();
    const available = this.signals.availableBytes;
    const capacity = this.signals.effectiveBytes ?? this.hardCap;
    const ratio = available !== undefined && capacity > 0 ? available / capacity : undefined;
    const psiCritical = this.signals.psiSomeAvg10 !== undefined && this.signals.psiSomeAvg10 >= 50;
    const psiPressure = this.signals.psiSomeAvg10 !== undefined && this.signals.psiSomeAvg10 >= 10;
    const critical = this.used > this.hardCap || (ratio !== undefined && ratio < 0.05) || psiCritical;
    const pressured = !critical && (this.used > this.target || (ratio !== undefined && ratio < 0.15) || psiPressure);
    if (critical) { this.state = 'critical'; this.target = Math.max(this.minMemory ?? 1, Math.floor(this.target * 0.7)); this.pressureAt = now; this.recoveryAt = undefined; }
    ...
    if (this.used > this.target) this.evictBounded(32);
...
export function resetDefaultMemoryBudget(): void { defaultBudget?.close(); defaultBudget = undefined; }
